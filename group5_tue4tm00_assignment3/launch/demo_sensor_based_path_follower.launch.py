...
def generate_launch_description():
    ...
    rviz_config = os.path.join(get_package_share_directory('group5_tue4tm00_assignment3'), 'config', 'robot_scan_map_costmap_path.rviz')
    robocyl_goal_map_launch = GroupAction(
        actions=[
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(get_package_share_directory('core_tue4tm00_robot_simulate'), 'launch', 'robocyl_goal_map_gazebo_turtlebot3_stage3.launch.py')
                )
            )
        ],
        scoped=True,
        ...
        actions = [
            GroupAction(
                actions=[
                    IncludeLaunchDescription(
                        PythonLaunchDescriptionSource(
                            os.path.join(get_package_share_directory('group5_tue4tm00_assignment3'), 'launch', 'safe_navigation_costmap.launch.py')
                        )
                    )
                ],
                scoped=True,
        ...
        actions = [
            GroupAction(
                actions=[
                    IncludeLaunchDescription(
                        PythonLaunchDescriptionSource(
                            os.path.join(get_package_share_directory('group5_tue4tm00_assignment3'), 'launch', 'sampling_based_path_planner.launch.py')
                        )
                    )
                ],
                scoped=True,
        ...
        actions = [
            GroupAction(
                actions=[
                    IncludeLaunchDescription(
                        PythonLaunchDescriptionSource(
                            os.path.join(get_package_share_directory('group5_tue4tm00_assignment3'), 'launch', 'sensor_based_path_follower.launch.py')
                        )
                    )
                ],
                scoped=True,
